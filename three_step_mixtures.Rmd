...
step1_fit <- mplusModeler(step1,
                            ...
                            modelout=here("manual_3step", "one.inp") ,
                            check=TRUE, run = TRUE, hashfilename = FALSE)
```
...
source("plot_lca.txt")
output_lsay <- readModels(here("manual_3step","one.out"))
...
step3_fit <- mplusModeler(step3,
               ...
               modelout=here("manual_3step", "three_starts.inp"), 
               check=TRUE, run = TRUE, hashfilename = FALSE)
```
...
```{r}
modelParams <- readModels(here("manual_3step", "three.out"))
...
step1_fit_bch <- mplusModeler(step1_bch,
                            dataout=here("bch_mplus", "Step1.dat"),
...
You'll see in the output that this model had trouble with convergence. This is common when applying the BCH method and another reason why we prefer to use ML three-step. 
...
step3_fit_bch <- mplusModeler(step3_bch,
               dataout=here("bch_mplus", "Step3.dat"), 
               modelout=here("bch_mplus", "Step3_bch.inp"), 
               check=TRUE, run = TRUE, hashfilename = FALSE)
```
...
Read in data from Step 1 of ML Method

```{r}
# Read in model with SAVEDATA information
output_lsay <- readModels(here("manual_3step","one.out"))
...
step3mod_fit <- mplusModeler(step3mod,
               dataout=here("lca_mod", "mod.dat"), 
               modelout=here("lca_mod", "Step3_mod_LSAY.inp"), 
               check=TRUE, run = TRUE, hashfilename = FALSE)
```
...
```{r}
modelParams <- readModels(here("lca_mod", "Step3_mod_LSAY.out"))
...
This is the intercept plotted across class. Here, the intercept is the expected value of math IRT scores when the predictor, `female`, is zero (and in this case 0 equals male)
...
# Save plot
ggsave(here("figures","Moderation_Interaction.jpeg"),              
       dpi=300, width=10, height = 7, units="in") 
```
